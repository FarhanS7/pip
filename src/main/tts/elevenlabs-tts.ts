/**
 * ElevenLabs Streaming TTS Provider (Task D.3)
 *
 * Implements TTSProvider using ElevenLabs high-quality speech synthesis.
 * Fetches audio/mpeg streams via Cloudflare Worker proxy (/tts) with X-Pip-Auth.
 *
 * References:
 *   PHASE_0_ARCHITECTURE.md §0.1 (main/tts module)
 *   PHASE_1_MODULES_AND_TASKS.md Task D.3 scoped checklist
 */
...
export class ElevenLabsTTSProvider implements TTSProvider {
  ...
  public readonly displayName = 'ElevenLabs Streaming TTS'
  ...
  public async speak(text: string): Promise<void> {
    ...
    this.stop()
    log.info('ElevenLabs TTS speak requested', { length: text.length })
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      audio.onended = () => {
        ...
        log.info('ElevenLabs TTS audio playback complete')
        resolve()
      }
      ...
      audio.onerror = (err) => {
        URL.revokeObjectURL(audioUrl)
        this.currentAudio = null
        log.error('ElevenLabs audio playback error', { error: String(err) })
  ...
  public stop(): void {
    if (this.currentAudio) {
      ...
      this.currentAudio = null
      log.info('ElevenLabs TTS playback stopped')
    }
  }
}
