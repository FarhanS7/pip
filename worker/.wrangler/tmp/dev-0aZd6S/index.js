...
// .wrangler/tmp/bundle-LbiDH3/checked-fetch.js
...
// .wrangler/tmp/bundle-LbiDH3/strip-cf-connecting-ip-header.js
...
var src_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      if ((url.pathname === "/transcribe-token" || url.pathname === "/transcribe-token/") && (request.method === "POST" || request.method === "GET")) {
...
async function handleChat(request, env) {
  ...
  const provider = bodyJson.provider || "claude";
  if (provider === "openai") {
    ...
    return new Response(response2.body, {
      ...
      headers: {
        ...CORS_HEADERS,
        "content-type": response2.headers.get("content-type") || "text/event-stream",
  ...
  if (provider === "gemini") {
    ...
    const model = bodyJson.model || "gemini-2.5-flash";
    delete bodyJson.provider;
...
async function handleTTS(request, env) {
  ...
  const provider = bodyJson.provider || "elevenlabs";
  if (provider === "openai") {
    if (!env.OPENAI_API_KEY) {
      return new Response(JSON.stringify({ error: "OPENAI_API_KEY not configured on Worker" }), {
        status: 500,
  ...
  const voiceId = bodyJson.voiceId || env.ELEVENLABS_VOICE_ID || "kPzsL2i3teMYv0FxEYQ6";
  delete bodyJson.provider;
...
// .wrangler/tmp/bundle-LbiDH3/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-LbiDH3/middleware-loader.entry.ts
