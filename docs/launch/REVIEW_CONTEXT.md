...
## B16: paid TTS transport and playback

OpenAI and ElevenLabs now share a main-process proxy transport: request cancellation, a 30-second fetch/body deadline, MPEG content validation, empty-body rejection and a 12 MiB audio ceiling. Main transfers the completed bytes to the singleton media renderer; the existing request-ID controller waits for actual end/error acknowledgements and bounds playback. Stop aborts pending fetch/playback. Renderer audio release pauses playback, detaches events, clears its source and revokes its blob URL. CSP permits blob media while retaining blocked renderer network connections. There are no main-process DOM audio calls or silent successful fallback.
...
## B19: multiple images and history

A shared message formatter labels each screenshot with its original screen number and attaches it exactly once to the latest user message. It preserves assistant/history text and each provider's existing image encoding. Duplicate screen IDs, more than four images, excessive aggregate image data and empty images are refused. The orchestrator sends all captured displays within these bounds and builds matching metadata. The legacy single-image payload remains compatible for callers but follows the same latest-message rule.

Evidence: 131 tests / 22 files plus lint, app/Worker/test/no-DOM types and production build pass. Request interception checks all three provider bodies for exactly-once image inclusion and correct labels across a history with earlier user turns; excessive input fails before fetch. No real screenshots or live API calls were used. Server validation (B05), display selection UX and real provider limits/certification remain pending, as does independent review. Rollback: revert B19 with Pip stopped; no storage changes.

## B20: streaming and stale targets

All AI adapters share a decoder that limits responses to 2 MiB, individual buffered events to 64 KiB, idle reads to 30 seconds and total reading to two minutes. It preserves split UTF-8, multiline data and CRLF, processes a final buffered event, and cancels/releases the reader on completion, failure, abort or consumer exit. Adapters recognize provider completion and fail on error events, malformed JSON or incomplete EOF; Gemini includes all text parts. Empty AI responses no longer enter successful history. Raw upstream HTTP bodies are no longer returned/logged by the response parser.

Point parsing refuses duplicate, invalid-screen and numerically unsafe tags and strips malformed/truncated tags from spoken text. Main rechecks captured display IDs and bounds before publishing a point. This does not yet prove native monitor-change timing or in-application element freshness.

Evidence: 140 tests / 23 files pass, with lint, app/Worker/test/no-DOM typechecks and production build. New fixtures cover byte-split Unicode, CRLF/multiline data, final EOF data, event limits, idle timeout/cancellation, corrupt/error/incomplete streams, all provider event formats, unsafe tags and moved/disconnected displays. Fetch-header timeouts remain B05/B06 transport follow-up; real providers and independent review remain unqualified. Rollback: revert B20 with Pip stopped; no stored data changes.
